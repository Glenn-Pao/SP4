...
CLoadGameState CLoadGameState::theLoadGameState;

void CLoadGameState::Init()
{
#if GSM_DEBUG_MODE
...
void CLoadGameState::Init(const int width, const int height, int level)
{
#if GSM_DEBUG_MODE
	cout << "CInstructionState::Init\n" << endl;
#endif
	scene = new CSceneLoadGame(width, height);
	scene->Init(level);
}

void CLoadGameState::Cleanup()
{
#if GSM_DEBUG_MODE
...
void CLoadGameState::HandleEvents(CGameStateManager* theGSM)
{
#if GSM_DEBUG_MODE
	/*int m_iUserChoice = -1;
...
void CLoadGameState::HandleEvents(CGameStateManager* theGSM, const unsigned char key, const bool status)
{
#if GSM_DEBUG_MODE
	/*int m_iUserChoice = -1;
...
void CLoadGameState::HandleEvents(CGameStateManager* theGSM, const double mouse_x, const double mouse_y,
...
void CLoadGameState::Update(CGameStateManager* theGSM)
{
#if GSM_DEBUG_MODE
...
void CLoadGameState::Update(CGameStateManager* theGSM, const double m_dElapsedTime)
{
	// Update the scene
	scene->Update(m_dElapsedTime);
}

void CLoadGameState::Draw(CGameStateManager* theGSM)
{
#if GSM_DEBUG_MODE
