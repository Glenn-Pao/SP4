	...
	const int width, const int height)
{
	...
	switch (scene->currentState)
	{
		...
		{
			if (button_Left == (int)true && scene->UIManager->FindButton("Next_Button")->getisHovered() == true)
			{
				...
				if (scene->numOfInstructionsLeft > 1)
				{
					...
					scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->getUI_List()[index], 0.f, Vector3(), 5.0f, UIAnimation::SCALING);
				...
				else
				{
					...
					// Close all window
					scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindImage("AlphaQuad"), 0.f, Vector3(), 5.0f, UIAnimation::SCALING);
		...
		{
			scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindImage("AlphaQuad"), 0.f, Vector3((float)scene->sceneManager2D.m_window_width, (float)scene->sceneManager2D.m_window_height, 1), 1.f, 2);
			if (scene->level == 0)
				scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindImage("TutScreen"), 0.f, Vector3(scene->sceneManager2D.m_window_width * 0.5f, scene->sceneManager2D.m_window_height * 0.6f, 1), 0.1f, 0);
			else
				scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindImage("WinScreen"), 0.f, Vector3(scene->sceneManager2D.m_window_width * 0.5f, scene->sceneManager2D.m_window_height * 0.6f, 1), 0.1f, 0);
			scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindButton("ReturnToHubButton"), 0.f, Vector3(scene->sceneManager2D.m_window_width * 0.5f, scene->sceneManager2D.m_window_width * 0.2f, 0), 0.1f, 0);
			...
			if (scene->UIManager->FindButton("ReturnToHubButton")->getisHovered() == true)
			{
				if (button_Left)
				{
					...
					if (theGSM->saveAndLoadsys->GetCurrentGameInfo()->DifficultySystems[0].getCurrentDifficultyUnlocked() <= scene->level)
					{
						theGSM->saveAndLoadsys->GetCurrentGameInfo()->DifficultySystems[0].setCurrentDifficultyUnlocked(scene->level + 1);
					}
					// Withdraw jellybean
		...
		case CSceneGame1::TIME_UP:
		{
			scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindImage("AlphaQuad"), 0.f, Vector3((float)scene->sceneManager2D.m_window_width, (float)scene->sceneManager2D.m_window_height, 1), 1.f, 2);
			if (scene->level == 0)
				scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindImage("TutLoseScreen"), 0.f, Vector3(scene->sceneManager2D.m_window_width * 0.5f, scene->sceneManager2D.m_window_height * 0.6f, 1), 0.1f, 0);
			else
				scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindImage("LoseScreen"), 0.f, Vector3(scene->sceneManager2D.m_window_width * 0.5f, scene->sceneManager2D.m_window_height * 0.6f, 1), 0.1f, 0);
			scene->UIManager->InvokeAnimator()->StartTransformation(scene->UIManager->FindButton("ReturnToHubButton"), 0.f, Vector3(scene->sceneManager2D.m_window_width * 0.5f, scene->sceneManager2D.m_window_width * 0.2f, 0), 0.1f, 0);
			// Return to hub Button
			if (scene->UIManager->FindButton("ReturnToHubButton")->getisHovered() == true)
			{
